...
class TrainHandler:
    def __init__(self, hparams):
        ...
        self.max_updates = hparams.get("max_updates", 160000)
        self.num_sanity_val_steps = hparams.get("num_sanity_val_steps", 10000)
        self.print_nan_grads = hparams.get("print_nan_grads", False)
        ...
        self.seed = hparams.get("seed", 3407)
        ...
        self.monitor_key = hparams.get("valid_monitor_key", 'val_loss')
        self.num_ckpt_keep = hparams.get("num_ckpt_keep", 3)
        self.save_best = hparams.get("save_best", True)
        monitor_mode = hparams.get("monitor_mode", 'min')
        ...
        self.proc_rank = 0
        # Tensorboard logging
        self.log_save_interval = hparams.get("log_save_interval", 100)
        self.val_check_interval = hparams.get("val_check_interval", 2000)
        self.tb_log_interval = hparams.get("tb_log_interval", 10)
        self.amp = hparams.get("amp", False)
        self.amp_scalar = GradScaler()

    def ddp_run(self, gpu_idx, task_cls, hparams_):
        self.hparams.update(hparams_)
        task = task_cls(hparams_)
    ...
    def handle(self, task_cls):
        ...
        if len(self.all_gpu_ids) > 1: # 有多个GPU则使用DDP
            ...
            self.task = task_cls(self.hparams) # 构建训练类对象
