import torch

from modules.commons.common_layers import Embedding, Linear
from modules.diffusion.teacher import PitchDiffusion
from modules.fastspeech.tts_modules import FastspeechEncoder, DurationPredictor, mel2ph_to_dur


class VariPredictor(torch.nn.Module):
    def __init__(self, ph_encoder, hparams):
        super(VariPredictor, self).__init__()
        self.encoder = FastspeechEncoder(
            ph_encoder=ph_encoder, 
            hidden_size=hparams['hidden_size'], 
            num_layers=hparams['enc_layers'], 
            kernel_size=hparams['enc_ffn_kernel_size'], 
            num_heads=hparams['num_heads']
        )
        # duration
        dur_hparams = hparams['dur_prediction_args']
        self.pred_dur = dur_hparams["enable"]
        if self.pred_dur:
            self.onset_embed = Embedding(2, hparams['hidden_size'])
            self.word_dur_embed = Linear(1, hparams['hidden_size'])
            self.dur_pred = DurationPredictor(
                in_dims=hparams['hidden_size'],
                n_layers=dur_hparams['num_layers'],
                n_chans=dur_hparams['hidden_size'],
                dropout_rate=dur_hparams['dropout'],
                kernel_size=dur_hparams['kernel_size'],
                offset=dur_hparams['log_offset'],
                dur_loss_type=dur_hparams['loss_type'],
            )
        else:
            self.dur_embed = Linear(1, hparams["hidden_size"])
        # f0
        f0_hparams = hparams['f0_prediction_args']
        self.pred_f0 = f0_hparams["enable"]
        # if self.pred_f0:
        #     self.f0_pred = PitchDiffusion()
    
    def forward(self, txt_tokens, onset, word_dur, mel2ph=None, infer=True):
        if self.pred_dur:
            extra_embed = self.onset_embed(onset)
            extra_embed += self.word_dur_embed(word_dur[:, :, None])
        else:
            assert mel2ph is not None
            dur = mel2ph_to_dur(mel2ph, txt_tokens.shape[1]).float()
            extra_embed = self.dur_embed(dur[:, :, None])
        encoder_out = self.encoder(txt_tokens, extra_embed)
        if self.pred_dur:
            ph_dur_pred = self.dur_pred(encoder_out, x_masks=txt_tokens == 0, infer=infer)
        # if self.pred_f0:
        #     pass
        return ph_dur_pred