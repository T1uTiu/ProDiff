...
class PitchPredictorTask(BaseTask):
    ...
    def run_model(self, sample, return_output=False, infer=False):
        note_midi = sample["note_midi"]  # [B, T_ph]
        mel2note = sample["mel2note"]
        f0 = sample["f0"]
        base_f0 = sample["base_f0"]
        spk_id = sample.get("spk_id", None)
        # 模型输出
        f0_pred = self.model(note_midi, mel2note, base_f0, f0, spk_id=spk_id, infer=infer)
        if infer:
            return f0_pred
        losses = {}
    ...
    def validation_step(self, sample, batch_idx):
        ...
        if batch_idx < self.hparams['num_valid_plots']:
            delta_f0_pred = self.run_model(sample, return_output=True, infer=True)
